...
class ImageBlobClient:
    ...
    """
    def __init__(self):
        self.storage_container_name = "cold-start-image-blobs"
    ...
    def __str__(self):
        return f"ImageBlobClient({self.storage_container_name})"
    ...
    async def initialize(self):
        ...
        if not self.storage_connection_string:
            raise ValueError(
                "ImageBlobClient failed to initialize: STORAGE_CONNECTION_STRING is missing."
            )
        if not self.storage_container_name:
            raise ValueError(
                "ImageBlobClient failed to initialize: STORAGE_CONTAINER_NAME is missing."
            )
        ...
        try:
            ...
            logger.info(f"ImageBlobClient initialized: {self.storage_container_name}")
    ...
    async def close(self):
        ...
        if self.blob_service_client:
            ...
            logger.info("ImageBlobClient connection closed")

    async def upload_image(self, file_name: str, file_bytes: bytes, content_type: str) -> str | None:
        """
        Use this for storing profile pictures, product images, and things like that.
        """
        if not self.container_client: 
            raise ValueError("ImageBlobClient Client not initialized. Call initialize() first.")
